...
class DataProcessor {
  ...
  Future<void> runFullProcess() async {
    ...
    try {
      ...
      final List<FollowUser> oldRelationsList = await _database.getNetworkRelationships(_ownerId);
      ...
      do {
        ...
        for (var userJson in followerResult.users) {
          final userId = userJson['id_str'] as String? ?? userJson['id']?.toString();
          if (userId != null) {
        ...
        _log("Fetched ${followerResult.users.length} followers, next cursor: $nextFollowerCursor");
      } while (nextFollowerCursor != null && nextFollowerCursor != '0' && nextFollowerCursor.isNotEmpty);

       _log("Finished fetching followers. Total unique users so far: ${newUserJsons.length}");
      ...
      do {
         ...
         _log("Fetched ${followingResult.users.length} following, next cursor: $nextFollowingCursor");
      } while (nextFollowingCursor != null && nextFollowingCursor != '0' && nextFollowingCursor.isNotEmpty);

      _log("Finished fetching following. Total unique users in combined list: ${newUserJsons.length}");
      ...
      _log("Calculated differences: ${addedIds.length} added, ${removedIds.length} removed, ${keptIds.length} kept.");

      _log("Processing ${removedIds.length} removed users to determine status...");
      ...
      if (removedIds.isNotEmpty) {
         ...
         for (final removedId in removedIds) {
            group.add(Future(() async {
               ...
               try {
                  final Map<String, dynamic> gqlJson = await _apiServiceGql.getUserByRestId(removedId, _ownerCookie);
                  ...
                  if (typename == 'User') {
                     ...
                     if (interstitial != null && interstitial.isNotEmpty) {
                        ...
                        if (wasFollower && wasFollowing) category = 'mutual_unfollowed';
                        else if (wasFollower) category = 'normal_unfollowed';
                        else if (wasFollowing) category = 'normal_unfollowed'; // <-- Per your request
                        else category = 'unknown_removed_state';
                     }
                  } else if (typename == 'UserUnavailable') {
                     category = 'suspended';
                  } else if (gqlJson['data']?['user'] == null || (gqlJson['data']?['user'] is Map && gqlJson['data']['user'].isEmpty)) {
                     category = 'deactivated';
                  } else {
                     _log("Warning: Unexpected GraphQL response for $removedId: $gqlJson");
      ...
      for (final keptId in keptIds) {
         ...
         final newJsonString = newJsonMap != null ? jsonEncode(newJsonMap) : null;
         final diffString = calculateReverseDiff(newJsonString, oldJsonString);
         if (diffString != null && diffString.isNotEmpty) {
            historyToInsert.add(FollowUsersHistoryCompanion(
      ...
      for (final userId in newIds) {
          ...
          // --- MODIFICATION END ---
          companionsToUpsert.add(FollowUsersCompanion(
              ownerId: Value(_ownerId),
              userId: Value(userId),
      ...
      for (final addedId in addedIds) {
          reportCompanions.add(ChangeReportsCompanion(
              ownerId: Value(_ownerId),
      ...
      await _database.transaction(() async {
         if (removedIds.isNotEmpty) {
           await _database.deleteNetworkRelationships(_ownerId, removedIds.toList());
           _log("Deleted ${removedIds.length} relationships from NetworkRelationships.");
         }
         if (companionsToUpsert.isNotEmpty) {
            await _database.batchUpsertNetworkRelationships(companionsToUpsert);
            _log("Upserted ${companionsToUpsert.length} relationships into NetworkRelationships.");
         }
         ...
         _log("Replaced ChangeReport with ${reportCompanions.length} new entries.");
      });
      _log("Analysis process completed successfully for account ID: $_ownerId.");
    } catch (e, s) {
      _log("!!! CRITICAL ERROR during analysis process for account ID: $_ownerId: $e");
      _log("Stacktrace: $s");
      rethrow;
    }
  }
