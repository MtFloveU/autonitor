...
class MediaProcessor {
  ...
  Future<MediaProcessingResult> processMedia({
    required Map<String, Map<String, dynamic>> newUsers,
    required Map<String, FollowUser> oldRelations,
  }) async {
    final List<Map<String, dynamic>> downloadTasks = [];

    for (final userId in newUsers.keys) {
      final userJson = newUsers[userId]!;

      // --- Avatar ---
      final String? avatarUrl = userJson['profile_image_url_https'] as String?;
      if (avatarUrl != null && avatarUrl.isNotEmpty && _settings.saveAvatarHistory) {
        downloadTasks.add({
          'userId': userId,
          'remoteUrl': avatarUrl,
      ...
      // --- Banner ---
      final String? bannerUrl = userJson['profile_banner_url'] as String?;
      if (bannerUrl != null && bannerUrl.isNotEmpty && _settings.saveBannerHistory) {
        downloadTasks.add({
          'userId': userId,
          'remoteUrl': bannerUrl,
...
Future<MediaProcessingResult> _runDownloadTasks(
    List<Map<String, dynamic>> downloadTasks) async {
  ...
  for (final task in downloadTasks) {
    ...
    group.add(Future(() async {
      ...
      try {
        final existingRecord = await _imageService.getMediaRecord(remoteUrl);
        String? localPath;
  ...
  _log("Finished downloading ${results.where((r) => r != null).length} items.");
